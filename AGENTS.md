...
- To clone an existing ordinary text file inside the same target PVF, read only `knowledge-pack/task-cards/pvf-same-pvf-file-copy-controlled-change.zh-CN.md` and `workspaces/examples/change-set.copy-pvf-file.example.json`. `copy-file` requires an absent same-extension target, rejects protected high-risk extensions, binds the complete source text and temporary independent readback, and does not mix replacement edits on the clone in the same round. Modify the clone only through a cumulative next round bound to the first successful `APPLY-MANIFEST.json`.
- For an existing `.nut` runtime edit, read only `knowledge-pack/task-cards/pvf-existing-nut-controlled-change.zh-CN.md` plus the routed NUT API quick entry. The dedicated route is ASCII-only, requires the pre-existing `load_state -> passive_skill -> appendage` chain with each target path used as a real function-call argument, original raw-text SHA256, exact target-PVF API evidence, function/APID audit, temporary independent PVF round-trip, final independent text and raw-byte SHA256 readback, and explicit in-game validation. It does not open existing `.co/.sqr/.str` or NUT text/StringLink writes.
...
- An authorized apply requires the matching unblocked dry-run record and approval code, an independent output, SHA256-verified content-addressed protected-source backup, and exact readback. Generation never overwrites its input. After that verified backup has become the protected-source anchor, separately authorized `client-pvf` deployment may replace the original path only when that path is the profiled client's live `Script.pvf`; the Agent must use the deployment lane instead of asking the user to copy files manually.
...
- Client deployment uses only `workbench.bat client-pvf`, requires separate authorization and client-closed confirmation, verifies a protected-source anchor, client backup, and installed hash, and offers controlled rollback. A profile source may initially be that same client's `Script.pvf`; once apply has promoted its verified content-addressed backup to the protected anchor, deployment installs the independent output automatically. It never authorizes NPK, IMG, UI, or other client resources.

Detailed text rules live only in the safety file. In brief: verified Chinese/Tw text must be a complete visible backtick token with target-confirmed encoding; bulk changes require exact positive counts; repeated text uses exact adjacent context, then exact non-overlapping `scope.startText`/`scope.endText`/`scope.expectedRanges` only when needed. Same-path parameter and text changes stay in one change-set and one final-file verification; preserve declared order for a dependent text-delete-renumber chain. Existing `.co`, `.sqr`, `.str`, StringLink display text and ordinary `.lst` edits remain blocked. Existing `.nut` remains blocked except for the exact `existing-nut-controlled-edit` ASCII-only runtime route described above; this route maps each selected text occurrence to one raw ASCII byte range, never re-encodes the whole NUT, and proves every non-target raw byte unchanged, so pre-existing undecodable Cn/Tw bytes may remain untouched. Its static pass never replaces in-game validation. A new `.co/.lst/.nut/.sqr/.str/.wdm` can proceed only through a matching `writeProof`, target-format audit, and temporary independent readback; an existing `.lst` row may use only the explicit `registry-lifecycle` route. Every touched text-bearing file still needs a client UI text check before deployment.

For integer/decimal parameter changes, select the complete tag plus its exact raw value, for example `[attack damage rate]\r\n1` to `[attack damage rate]\r\n0.8`; never use a bare repeated `1` or `0.8`. This is an ordinary numeric/raw-token replacement and does not require a text mode.
